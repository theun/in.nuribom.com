...
<div id="top-toolbar">
    <h3>
        ...
        <a href="${request.route_path('admin_group_edit', id=group.id)}">${group.name.split(':')[1]}</a> > 
        인원 추가
    </h3>
...
function doAdd() {
    ...
    var url = "${request.route_path('admin_group_member_add', id=group.id)}";
    $.post(url, {"id-list":data.join()}, function() {
        $(location).attr("href", "${request.route_path('admin_group_edit', id=group.id)}");
    }, "json");
}
...
function toggleSort(e) {
    var reverse = false;
    var url = "/admin/group/${group.id}/member_add?sort=" + $(this).prop("id"); 
    console.log('url');
